...
farm_df = pd.read_pickle('3tier_meta.pickle')
df_hist = pd.read_pickle('3tier_hist.pickle')
...
st.title('State Level Wind Generation Boxplots')
...
# Open a select menu 
cols = st.columns(6)
states = cols[0].multiselect("Select State", list(state_map.values()), default = ['Texas', 'Iowa', 'Illinois'])
...
start_date = cols[4].selectbox("Select first day", list((range(1,31))))
end_date = cols[5].selectbox("Select end day", list((range(1,31))))


# Set masks to load data in range 
begin= now.replace(month = months, day=start_date, hour = 0, minute=0, second=0, microsecond=0)
end = now.replace(month = months,day=end_date+5, hour = 23, minute=0, second=0, microsecond=0)

# Set masks to filter actuals data by day 
begin_act = begin.strftime('%m-%d')
end_act = end.strftime('%m-%d')


# Set masks to load data from beginning of selected month - NEED TO GET DATA UP UNTIL CURRENT DAY
begin = now.replace(month = 1, day=1, hour = 0, minute=0, second=0, microsecond=0)
end = now.replace(hour = 0, minute=0, second=0, microsecond=0)

# 
# Read in actuals
recent_3t_df = three_tier_wind_gen_actuals(begin, end, iso=['miso', 'spp', 'pjm'])
# Read in actuals
recent_3t_df = three_tier_wind_gen_actuals(begin, end, iso=['miso', 'spp', 'pjm'])
# joined the 3tier dataset to the farm metadata, so we can group wind gen by state
joined_recent = recent_3t_df.set_index('three_tier_farm_name').join(farm_df.set_index('three_tier_farm_name'), lsuffix='_3t')
df_recent = joined_recent[~joined_recent.state.isna()]
df_month = db_to_df(df_recent)
df_month.loc[:,'pct_mw'] = df_month.mw / df_month.max_capacity
mask_recent = (df_month['date'] >= begin_act) & (df_month['date'] <= end_act)
mask_recent = df_month.loc[mask_recent]

act_state = mask_recent
df_act_st = act_state[act_state.state.isin(states)]

df1 = df_act_st.groupby(['hour', 
                   'day', 'state'])['mw'].sum().reset_index()

df_pctmw = df_act_st.groupby(['hour', 
                   'day', 'state'])['pct_mw'].mean().reset_index()
...
fig, axes = plt.subplots(len(states), 1, dpi=600, figsize=(len(states)*2,len(states)*4), sharey=False)
